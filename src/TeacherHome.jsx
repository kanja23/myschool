...
const ROLE_LABEL = { superadmin:'Super Admin', admin:'School Admin', teacher:'Teacher', student:'Student', parent:'Parent', supplier:'Supplier' }
const ROLE_COLOR = { superadmin:'bg-purple-100 text-purple-800', admin:'bg-primary-100 text-primary-800', teacher:'bg-blue-100 text-blue-800', student:'bg-amber-100 text-amber-800', parent:'bg-pink-100 text-pink-800', supplier:'bg-gray-100 text-gray-700' }
  ...
  return (
    <div className="min-h-screen flex bg-surface-100">
      ...
      <aside className={clsx(
        ...
        {/* School name */}
        {profile?.schoolName && (
          <div className="px-5 py-2.5 border-b border-primary-800">
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          {navItems.map(item => (
            <NavLink key={item.path} to={item.path} end={item.end}
              ...
              {item.badge && (
                <span className="ml-auto bg-accent-500 text-white text-2xs font-bold px-1.5 py-0.5 rounded-full">{item.badge}</span>
              )}
            </NavLink>
          ))}
        </nav>
        ...
        <div className="px-4 py-4 border-t border-primary-800 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <span className="text-primary-400 text-2xs uppercase tracking-wide font-medium">Notifications</span>
            <NotificationPanel />
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <span className={clsx('badge text-2xs', ROLE_COLOR[profile?.role])}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-surface-200 px-5 h-14 flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-3">
            <button className="lg:hidden p-1.5 rounded-lg hover:bg-surface-100 text-gray-500" onClick={() => setSidebarOpen(v => !v)}>
              <MenuIcon size={18} />
            </button>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-surface-100 rounded-lg">
              ...
              <span className="text-xs font-semibold text-gray-700">{profile?.currentTerm ?? 1} · {new Date().getFullYear()}</span>
            </div>
          </div>
        </header>
        <main className="flex-1 p-5 md:p-6 overflow-auto animate-fade-in">{children}</main>
      </div>
    </div>
  )
}
